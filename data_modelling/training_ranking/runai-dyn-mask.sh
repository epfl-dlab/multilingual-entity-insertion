...
&& \
echo "HF_HOME: $HF_HOME" > runai_log_2.txt \
&& \
echo "MODEL_NAME: $MODEL_NAME" >> runai_log_2.txt \
&& \
echo "MODEL_ARCHITECTURE: $MODEL_ARCHITECTURE" >> runai_log_2.txt \
&& \
source .bashrc_wendler \
&& \
echo "New .bashrc loaded" >> runai_log_2.txt \
&& \
conda activate /dlabdata1/tsoares/conda/envs/runai/ \
&& \
echo "Conda environment activated" >> runai_log_2.txt \
&& \
